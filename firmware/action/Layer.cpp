...
namespace action {
Layer::Layer(
    const std::string &i_name, const int *i_color,
...
Layer::LoadDefinition(const std::string &switch_uid,
  ...
  return
      [switch_uid, layer_name, is_toggle](logic::quantum::Timeline &timeline) {
...
void Layer::on_press(logic::quantum::Timeline &timeline,
  ...
  DEBUG_INFO("New logic::quantum::Timeline %s events after load: %d (@%d)",
...
void Layer::add_to_timeline(logic::quantum::Timeline &timeline) {
  DEBUG_INFO("logic::quantum::Timeline::add_to_timeline");
  for (const auto &pair : keys) {
    const std::string pressed_id = pair.first;
    const std::function<void(logic::quantum::Timeline &)> function =
        pair.second;
